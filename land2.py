...
input_lang, output_lang, pairs, MAX = prepareData('ori', 'recon')
...
def train(input_tensor, target_tensor, embed, encoder, decoder, embed_optimizer, encoder_optimizer, decoder_optimizer, criterion, max_length=MAX_LENGTH):
    ...
    all_weight = torch.sum(torch.exp(weights))
    
    #print(weights)
    
    for ei in range(input_length):
        encoder_output, encoder_hidden = encoder(
            embeddings[ei], encoder_hidden, torch.exp(weights[ei]) / all_weight)
...
def trainIters(embed, encoder, decoder, n_iters, print_every=1000, plot_every=100, learning_rate=0.0001):
    start = time.time()
    plot_losses = []
    ...
    with torch.no_grad():
        ...
        for ei in range(input_length):
            encoder_output, encoder_hidden = encoder(embeddings[ei],
                                                     encoder_hidden, torch.exp(weights[ei]) / all_weight)
            encoder_outputs[ei] += encoder_output[0, 0]
...
def evaluateRandomly(embed, encoder, decoder, n=10):
    acc = 0
    total = 0
...
trainIters(embed1, encoder1, attn_decoder1, 60000, print_every=1000)

evaluateRandomly(embed1, encoder1, attn_decoder1)
